...
- ui: l'ombra di `glassSurface` sa da che parte sta. Su una pagina scura un'ombra nera e' un quadrato nero su un pavimento nero e non fa niente, e a sollevare il pannello ci pensa la pellicola; sul chiaro e' il contrario, perche' la pellicola non ha piu' spazio sopra di se', quindi ombra e capello sono tutto quello che dice che una superficie galleggia. Mezzo peso era il valore scelto quando esisteva un lato solo.
- theme: `FluidTheme` accetta `systemBars`. Un tema che *e'* lo schermo rivendica le barre; uno applicato a una porzione — la testata di un disco, che e' una superficie scura per i controlli che ci stanno sopra — non ha titolo per decidere che aspetto ha l'orologio. Lasciato libero vince il tema piu' interno che ha ricomposto per ultimo, e un'app chiara che mostrava una copertina scura si ritrovava i glifi bianchi su una pagina bianca.
- fix: due trappole di luminanza. `FluidNotification` chiedeva `surface.luminance()`, ma `surface` in un'app costruita su questo design e' una pellicola traslucida e `luminance` ignora l'alpha: rispondeva «chiaro» per la configurazione piu' nera che esista, e il ramo sotto era codice morto. `FluidComponents` chiedeva `background.luminance()`, e un'app che dipinge un'immagine dietro le pagine lascia `background` trasparente: luminanza zero, quindi diceva «scuro» da tutti e due i lati e il tema chiaro riceveva contenitori scuri con dentro testo chiaro. Tutte e due adesso chiedono a `GlassDefaults.isDarkSurface()`, che e' la risposta che il tema conosce e che non si puo' falsificare.
...
- ai: il modello puo' chiedere da se' il livello profondo
