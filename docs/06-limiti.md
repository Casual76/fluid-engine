...
- **Non ogni app puo' ospitarlo, e per due di loro c'e' una risposta.** `engine-ui` e' scritto su Material 3 androidx: un'app in Compose Multiplatform (`org.jetbrains.compose`) non puo' prenderlo, e un'app di sole View non ha niente da tematizzare. In entrambi i casi i moduli senza UI (aggiornamento in-app, config remota) girano lo stesso, e nel caso Multiplatform il look si puo' **portare** a mano dichiarandolo (`engine.mode=port`), cosi' resta contato quando l'engine si muove. Un AGP vecchio invece non e' un ostacolo: si abbassa il compileSdk dell'engine dal `gradle.properties` dell'app. I requisiti sono in [`01-integrazione.md`](01-integrazione.md).
